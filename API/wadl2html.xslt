...
-->
    ...
    <xsl:template match="wadl:resource" mode="toc">
        ...
        <li><a href="#{$id}">
				<!-- CSSI -->
				<xsl:choose>
					<xsl:when test="wadl:doc/comment">
						<comment><xsl:value-of select="$name"/></comment>
					</xsl:when>
					<xsl:when test="wadl:doc/version">
						<version><xsl:value-of select="$name"/></version>
					</xsl:when>                                        
					<xsl:otherwise>
            ...
            <xsl:otherwise>               
                <li>
                    <a href="#{$id}">
						...
						<xsl:choose>
							<xsl:when test="wadl:doc/comment">
								<comment><xsl:call-template name="representation-name"/></comment>
							</xsl:when>
							<xsl:when test="wadl:doc/version">
								<version><xsl:call-template name="representation-name"/></version>
							</xsl:when>                                                        
							<xsl:otherwise>
